...
Although the optimisation strategy described above is probably effective in most of the cases, in same cases it can be useful - in order to reduce the amount of HTTP requests - to place compressed assets in-line, directly in the HTML file (as opposed to placing them into external stylesheets ). Sqweeze provides in fact an experimental functionality for compiling assets inline into the DOM document (This is still experimental and rather useless at the moment, as it does not support base64 image embedding yet). 
